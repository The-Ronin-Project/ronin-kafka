...
/**
 * Occurs with [com.projectronin.kafka.RoninConsumer]s when one of the [Ronin Event Standard]
 * fields is missing from the Kafka record headers.
 *
 * See Also:
...
class DeserializationException(val type: String, valueClass: KClass<*>) :
    RuntimeException(
        "There was an exception attempting to deserialize " +
            "the message for the type $type into the class $valueClass"
    )
