 ...
 */
class EventHeaderMissing(val topic: String?, missingHeaders: List<String>) :
    RuntimeException("Unable to process event. The following headers are required: ${missingHeaders.joinToString(", ")}")

/**
 * Occurs when a Kafka message is received with a `ce_type` header that the [com.projectronin.kafka.serde.RoninEventDeserializer]'s
 * typeMap does not have an entry for.
 */
class UnknownEventType(val topic: String?, val key: String, val type: String?) :
