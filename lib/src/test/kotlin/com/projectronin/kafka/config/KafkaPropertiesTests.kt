...
class KafkaPropertiesTests {
    ...
    fun `provides defaults`() {
        val properties = KafkaProperties().properties
    ...
    fun `overrides defaults`() {
        val defaults = KafkaProperties().properties
        val overridden = KafkaProperties("linger.ms" to 42).properties
    ...
    fun `appends new property`() {
        val defaults = KafkaProperties().properties
        val appended = KafkaProperties("the.answer" to 42L).properties
