...
class RoninConsumer(
    ...
    private val kafkaConsumer: KafkaConsumer<String, String> = KafkaConsumer(kafkaProperties.properties),
     ...
     */
    private fun handleEvent(event: RoninEvent<*>, handler: (RoninEvent<*>) -> RoninEventResult): Boolean {
        try {
     ...
     * currently required for the creation of a [RoninEvent]).
     *
     * @param record The ConsumerRecord<String, String> as received from Kafka
     ...
     */
    private fun parseEvent(record: ConsumerRecord<String, String>): RoninEvent<*>? =
        try {
            val headers = parseHeaders(record.headers())
            validateHeaders(headers)

            val type = headers.getValue(KafkaHeaders.type)
            when (val it = typeMap[type]) {
                null -> throw UnknownEventType(record.key(), type)
